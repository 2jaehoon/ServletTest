...
<!-- DD에 정의된 URL패턴<url-pattern>을 action 속성의 값으로 설정한다. -->
<form action="../param_test" method="get"  name="getFrm" enctype="application/x-www-form-urlencoded">
...
HTML Form Control의 name속성이 유일한 경우의 처리.<br/>
<form action="http://localhost/servlet_prj/param_test" method="post" name="postFrm" >
<label>이름</label> <input type="text" name="name" class="inputBox" /><br/>
<label>나이</label> <input type="password" name="age" class="inputBox" /><br/>
<label>주소</label> <input type="hidden" name="addr" value="서울시 강남구 역삼동" /><br/>
<label>도메인</label>
